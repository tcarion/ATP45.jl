...
function ATP_line(lat, lon, line_length, we_comp, sn_comp, resolution)::Feature
	...
	lonL = [lon, lon + line_length / (EARTH_RADIUS * cos(lat * pi / 180)) * 180 / pi * we_comp / norm]
	latL = [lat, lat + line_length / EARTH_RADIUS * 180 / pi * sn_comp / norm]
...
function ATP_line_angle(lat, lon, line_length, alpha)::Feature
	...
	lonL = [lon, lon + line_length / (EARTH_RADIUS * cos(lat * pi / 180)) * 180 / pi * cos(alpha * pi / 180)]
	latL = [lat, lat + line_length / EARTH_RADIUS * 180 / pi * sin(alpha * pi / 180)]
